...
async function verify(tarball, distTag) {
  ...
  try {
    ...
    await mkdir(fresh);
    await writeFile(join(fresh, 'package.json'), JSON.stringify({ name: 'treedatagrid-public-registry-consumer', version: '1.0.0', private: true, type: 'module' }));
    ...
    const smoke = spawnSync(process.execPath, ['--input-type=module', '-e', `import assert from 'node:assert/strict'; import { ObservableList, FlatTreeDataGridSource } from ${JSON.stringify(pkg.name)}; import * as core from ${JSON.stringify(pkg.name + '/core')}; assert.equal(ObservableList, core.ObservableList); const model = new FlatTreeDataGridSource(new ObservableList([1,2])); assert.equal(model.Rows.Count,2); model.Dispose();`], { cwd: fresh, stdio: 'inherit', timeout: 30_000 });
    assert.equal(smoke.status, 0, 'Fresh npm installation must execute the published Core model');
    ...
    await summary(`Published and verified **${pkg.name}@${pkg.version}** on npm (${distTag}).\n\nRelease and registry tarballs have identical SHA512 integrity. Public download, provenance metadata, ESM/CommonJS/TypeScript consumers, shared Core constructor identity, CSS and standalone assets, and fresh npm installation checks passed.\n\n[View npm package](https://www.npmjs.com/package/${pkg.name}/v/${pkg.version})`);
  } finally {
